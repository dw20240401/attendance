...
document.getElementById("stampingTime").disabled = true;
...
function radioButton2() {
    document.getElementById("stampingTime").disabled = false;
    var stampingTime = document.getElementById("stampingTime").value;
    console.log(stampingTime);
}
...
document.querySelector('#startWorkBtn').addEventListener('click', function() {
    ...
    if (document.getElementById("radio2").checked) {
        // 打刻時刻チェック
        if (document.getElementById("stampingTime").value != "") {
            var time = document.getElementById("stampingTime").value + ":00";
            var stampingDatetime = year + "/" + month  + "/" + day + " " + time;
            // ISO8601
            var isoDate = toISOStringWithTimezone(new Date(stampingDatetime));
        } else {
            alert("打刻時刻が入力されていません。");
            document.getElementById("startWorkBtn").disabled = false;
            document.getElementById("endWorkBtn").disabled = false;
            return;
        }
                ...
                timerecordDocRef.get().then((timerecordDoc) => {
                    if (timerecordDoc.exists) {
                        ...
                        db.collection("attendance2").doc(userUid.value).collection("timerecord").doc(date).update({
                            ...
                            // メッセージモーダル
                            document.getElementById("modalMessage").innerHTML = "勤務開始登録しました。<br>勤務開始時刻：" + time;
                ...
                timerecordDocRef.get().then((timerecordDoc) => {
                    if (timerecordDoc.exists) {
                        ...
                        db.collection("attendance2").doc(userUid.value).collection("timerecord").doc(date).update({
                            ...
                            // メッセージモーダル
                            document.getElementById("modalMessage").innerHTML = "勤務終了登録しました。<br>勤務終了時刻：" + time;
                            messageModal.style.display = "block";
                            document.getElementById("startWorkBtn").disabled = false;
                            document.getElementById("endWorkBtn").disabled = false;
                        }).catch((error) => {
